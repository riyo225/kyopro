...
int main() {
    ...
    ll N = s.size();
    ll r = 0;
    vector<pair<char, ll>> ress;
    for (ll l = 0; l < N; ) {
        while (r < N && (s[l] == s[r])) {
            r++;
        }
        ress.push_back({s[l], r - l});
        l = r;
    }

    N = t.size();
    r = 0;
    vector<pair<char, ll>> rest;
    for (ll l = 0; l < N; ) {
        while (r < N && (t[l] == t[r])) {
            r++;
        }
        rest.push_back({t[l], r - l});
        l = r;
    }
    ...
    if (ress.size() != rest.size()) {
        ...
        rep(i, ress.size()) {
            // cout << ress[i].first << rest[i].first << nl;
            if (ress[i].first != rest[i].first) {
                ans = false;
            }
            else {
                ll sub = rest[i].second - ress[i].second;
                if (sub != 0 && ress[i].second == 1) ans = false;
                if (sub < 0) ans = false;
            }
        }
    }
