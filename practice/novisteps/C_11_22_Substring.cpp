...
int main() {
    ...
    // string t = RLE::decode<string>(res);
    // cout << t << nl;

    ll ans = 1;
    for (int i = 1; i < sz(res)-1; i++) {
        if (res[i].first == '/' && res[i].second == 1) {
            if (res[i-1].first == '1' && res[i+1].first == '2') {
                chmax(ans, min(res[i-1].second, res[i+1].second) * 2 + 1);
            }
        }
    }
    cout << ans << nl;
