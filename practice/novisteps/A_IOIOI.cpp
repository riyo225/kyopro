...
int main() {
    ...
    auto check = [&](ll l, ll r) -> bool {
        if (r+1 > l + 2*n || r+1 > sz(res)) return false;
        if (r > l && res[r].second != 1) return false;
        if (res[l].first != 'I') return false;
        if (r+1 == l + 2*n) {
            if (res[r+1].first != 'I') return false;
        }
        else {
            if ((r+1-l) % 2 == 1) {
                if (res[r+1].first != 'O' || res[r+1].second != 1) return false;
            }
            else {
                if (res[r+1].first != 'I' || res[r+1].second != 1) return false;
            }
        }
        return true;
    };
    
    for (ll l = 0; l <= sz(res)-(2*n+1); l++) {
        while (r < sz(res) && check(l, r)) {
            r++;
        }
        if (r - l == 2 * n) {
            ans++;
