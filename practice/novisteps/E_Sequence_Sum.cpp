...
struct Doubling {
    ...
    Doubling(const vector<T>& next_states) {
        n = next_states.size();
        dp.assign(LOG, vector<T> (n, -1));

        for (ll i = 0; i < n; i++) {
            dp[0][i].first = next_states[i];
            dp[0][i].second = i;
        }
        ...
        for (ll k = 0; k < LOG - 1; k++) {
            for (ll v = 0; v < n; v++) {
                if (dp[k][v].first == -1) {
                    ...
                    dp[k + 1][v].first = dp[k][dp[k][v].first].first;
                    dp[k + 1][v].second = dp[k][dp[k][v]].second + 
                }
            }
        }
    }
