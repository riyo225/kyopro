...
int main() {
    ...
    rep(i, n) cin >> cost[i][0] >> cost[i][1] >> cost[i][2];

    vector<vector<ll>> memo(n+1, vector<ll> (3, -1));
    auto rec = [&](auto self, ll x, ll v) -> ll {
        if (x == 0) return cost[0][v];

        if (memo[x][v] != -1) return memo[x][v];

        ll max_prev = 0;
        for (int i = 0; i < 3; i++) {
            if (i == v) continue;

            chmax(max_prev, self(self, x - 1, i));
        }
        
        return memo[x][v] = max_prev + cost[x][v];
    };

    cout << max({rec(rec, n-1, 0), rec(rec, n-1, 1), rec(rec, n-1, 2)}) << nl;
