...
int main() {
    ...
    while (t--) {
        ...
        deque<ll> dq;
        rep(i, n) {
            if ((a[i] % 2 == 1) && (a[i] % 3 == 1 || a[i] % 3 == 2)) dq.push_back(a[i]);
        }

        rep(i, n) {
            if (a[i] % 2 == 0 && (a[i] % 3 == 1 || a[i] % 3 == 2)) dq.push_back(a[i]);
            if (a[i] % 2 == 1 && a[i] % 3 == 0) dq.push_front(a[i]); 
        }
        rep(i, n) {
            if (a[i] % 6 == 0) dq.push_front(a[i]);
        }
        while (!dq.empty()) {
            cout << dq.front() << " ";
            dq.pop_front();
