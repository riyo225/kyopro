...
int main() {
    ...
    for (int i = 0; i < sz(suma); i++) {
        if (s - suma[i] < 0) continue;
        auto itra = lower_bound(all(sumb), s - suma[i]);
        auto itrb = upper_bound(all(sumb), s - suma[i]);
