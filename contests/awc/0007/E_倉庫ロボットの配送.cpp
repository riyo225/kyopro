...
int main() {
    ...
    vector<vector<ll>> dist(m+2, vector<ll> (m+2, 0)); 
    rep(i, m+2) {
        rep(j, m+2) {
            ll r1 = p[i] / n;
            ll c1 = p[i] % n;
    ...
    vector<vector<ll>> dp(1LL << m+2, vector<ll> (m+2, INF_LL));
    dp[1][0] = 0;
    for (ll bit = 0; bit < (1 << m+2); bit++) {
        for (ll v = 0; v < m+2; v++) {
            if (!(bit >> v & 1)) continue;
            if (dp[bit][v] == INF_INT) continue;

            for (int nv = 0; nv < m+2; nv++) {
                if (bit >> nv & 1) continue;

                ll nbit = bit | (1LL << nv);
                dp[nbit][nv] = min(dp[nbit][nv], dp[bit][v] + dist[v][nv]);
            }
        }
    }
    // debug_2d_vec(dp);

    cout << dp[(1LL << m+2) - 1][m+1] << nl;
