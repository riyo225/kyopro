...
int main() {
    ...
    ll max_ = min(v[0], v[2]) + 1;
    chmin(max_, v[1]);

    Combination comb(1000001);

    vector<mint> f(max_+1, 0);
    f[1] = 2;
    for (int i = 2; i <= max_; i++) {
        f[i] += comb.nCr(v[0]-1, (i - 1) / 2) * comb.nCr(v[2]-1, (i) / 2);
        f[i] += comb.nCr(v[2]-1, (i - 1) / 2) * comb.nCr(v[0]-1, (i) / 2);
    }

    mint ans = 0;
    for (int i = 1; i <= max_; i++) {
        if (v[1] - i >= 0) ans += f[i] * comb.nCr(m+1, v[1] - i);
    }
