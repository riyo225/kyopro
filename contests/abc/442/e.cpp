...
int main() {
    ...
    vector<Monster> xy(n);
    for (int i = 0; i < n; i++) {
        xy[i] = {x[i], y[i], i + 1, atan2l(y[i], x[i])};
    }

    sort(all(xy), [](const Monster &p1, const Monster &p2) {
        return p1.angle < p2.angle;
    });

    for (int i = 0; i < n; i++) {
        cout << "(" << xy[i].x << ", " << xy[i].y << ") : " << xy[i].angle << endl;
    }
    ...
    while (q > 0) {
        ld a, b;
        cin >> a >> b;
        auto s = lower_bound(all(xy), atan2l(y[b-1], x[b-1]));
        auto e = upper_bound(all(xy), atan2l(y[a-1], x[a-1]));
