    ...
    while (true) {
        ...
        rep(i, n) {
            ...
            if (i == 0) {
                ...
                if (sub[i].x >= sub[i-1].x && sub[i].x >= sub[i+1].x) {
                    x = 1;
                }
                else if (sub[i].x < sub[i-1].x && sub[i].x < sub[i+1].x) {
                    x = 2;
                }
            }
        ...
        rep(i, N) {
            if (i == 0) {
                chmin(ans, fsum - bsum);
                if (!floor.empty()) {
                    fsum -= sub[floor.front()].x;
                    floor.pop_front();
                }
                if (!floor.empty()) {
                    fsum -= sub[floor.back()].x;
                    floor.pop_back();
                }
            }
            else if (i % 2 == 0) {
                chmin(ans, fsum + m * sz(floor) - bsum);
                if (!floor.empty()) {
                    fsum -= sub[floor.front()].x;
                    floor.pop_front();
                }
                ...
                chmin(ans, bsum + m * sz(bottom) - fsum);
                if (!bottom.empty()) {
                    bsum -= sub[bottom.front()].x;
                    bottom.pop_front();
                }
                if (!bottom.empty()) {
                    bsum -= sub[bottom.back()].x;
                    bottom.pop_back();
                }
            }
            if (floor.empty() || bottom.empty()) break;
        }
        cout << ans << nl;
    }
