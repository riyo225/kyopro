...
int main() {
    ...
    for (int i = 0; i < sz(t); i++) {
        ...
        if (t[i] == 'U' && is_inside(x-1, y) && vec[x-1][y] != '#') {
            x--;
        }
        else if (t[i] == 'D' && is_inside(x+1, y) && vec[x+1][y] != '#') {
            x++;
        }
        else if (t[i] == 'L' && is_inside(x, y-1) && vec[x][y-1] != '#') {
            y--;
        }
        else if (t[i] == 'R' && is_inside(x, y+1) && vec[x][y+1] != '#') {
            y++;
        }
    }
    if (vec[x][y] == '@' && !seen[x][y])ans++;
