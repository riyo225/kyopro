...
int main() {
    ...
    rep(i, n) {
        auto itr = lower_bound(all(b), make_pair(a[i], make_pair(-1LL, 0LL)));
        ...
        while (b[idx].second.first > i) {
            b[idx].second.first = i;
            idx++;
        }
    ...
    rep(i, m) {
        if (b[i].second.first == INF_LL) {
            cout << -1 << nl;
        }
        else {
            cout << b[i].second.first + 1 << nl;
        }
    }
