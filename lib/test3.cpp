...
Node* expression(State &begin) {
    if (isdigit(*begin)) {
        ll v = *begin - '0';
        begin++;
        return new Node{true, v, "", nullptr, nullptr};
    }

    consume(begin, 'i'); consume(begin, 'f'); consume(begin, '_');
    ...
    consume(begin, '_'); consume(begin, 't'); consume(begin, 'h');
    consume(begin, 'e'); consume(begin, 'n'); consume(begin, '_');

    Node* left_node = expression(begin);

    consume(begin, '_'); consume(begin, 'e'); consume(begin, 'l');
    consume(begin, 's'); consume(begin, 'e'); consume(begin, '_');

    Node* right_node = expression(begin);

    return new Node{false, 0, var, left_node, right_node};
}
...
int main() {
    ...
    auto dfs = [&](auto self, Node* node) -> mint {
        if (node->is_leaf) {
            ll k = t.size() + f.size();
            return ((mint)node->val * two.pow(m - k));
        }

        mint res = 0;
        string v = node->var_name;
